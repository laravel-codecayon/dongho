{{--*/ usort($tableGrid, "SiteHelpers::_sort") /*--}}
  <div class="page-content row">
	...
	{{ $details }}
	 ...
	 {{ Form::open(array('url'=>'post/destroy/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) }}
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
					...
					{{--*/ $id = SiteHelpers::encryptID($row->post_id) /*--}}
				 	@if($access['is_detail'] ==1)
					<a href="{{ URL::to('post/show/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"  class="tips btn btn-xs btn-primary"  title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('post/add/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="fa fa-edit"></i></a>
					@endif
