{{--*/ usort($tableGrid, "SiteHelpers::_sort") /*--}}
  <div class="page-content row">
    ...
    <div class="toolbar-line ">
			...
			<a href="{{ URL::to('module/config/Customer') }}" class="tips btn btn-xs btn-default"  title="{{ Lang::get('core.btn_config') }}">
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
					...
					{{--*/ $id = SiteHelpers::encryptID($row->customer_id) /*--}}
				 	@if($access['is_detail'] ==1)
					<a href="{{ URL::to('Customer/show/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"  class="tips btn btn-xs btn-primary"  title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('Customer/add/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="fa fa-edit"></i></a>
					@endif
...
$(document).ready(function(){
	...
	$("#export-excel").click(function(){
		$('#SximoTable').attr('action','{{ URL::to("Customer/downloads")}}');
		$('#SximoTable').submit();
	});
});	
</script>		