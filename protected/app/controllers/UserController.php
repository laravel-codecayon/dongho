...
class UserController extends BaseController {
	...
	function autoSigninCus($cus,$profile){
		if(count($cus) > 0){
			if($cus->status == 0){
				return Redirect::to('thong-bao.html')->with('message', SiteHelpers::alert('success','Tài khoản của bạn tạm thời đã bị khóa !'));
			}
			...
			return Redirect::to('dang-ky.html')->with('message_dangky', SiteHelpers::alert('success','Bạn vui lòng xác nhận các thông tin bên dưới !  '))->with('input_rd',$input);
		}
